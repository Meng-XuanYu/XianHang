...
public class BuyActivity extends AppCompatActivity {
    ...
    private List<GetMyBuyResponse.Trade> trades;
...
//        }
        ...
        // 发送 POST 请求
        apiService.getMyBuy(userId).enqueue(new Callback<GetMyBuyResponse>() {
            @Override
            public void onResponse(Call<GetMyBuyResponse> call, Response<GetMyBuyResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    GetMyBuyResponse getMyBuyResponse = response.body();
                    if ("success".equals(getMyBuyResponse.getStatus())) {
                        trades = getMyBuyResponse.getTrades();
                    } else {
                        // 登录失败
                        Toast.makeText(BuyActivity.this, "getMyBuyResponse.getMessage()", Toast.LENGTH_SHORT).show();
                        Log.e("MyBuy", "Error: " + "getMyBuyResponse.getMessage()");
                    }
                } else {
                    try {
                        // 从 errorBody 获取错误信息
                        ...
                        Toast.makeText(BuyActivity.this, errorMessage, Toast.LENGTH_SHORT).show();
                        Log.e("MyBuy", "Error: " + errorMessage);
                    } catch (Exception e) {
                        Toast.makeText(BuyActivity.this, "解析错误消息失败", Toast.LENGTH_SHORT).show();
                        Log.e("MyBuy", "Error parsing error body: ", e);
                    }
                }
            }

            @Override
            public void onFailure(Call<GetMyBuyResponse> call, Throwable t) {
                Toast.makeText(BuyActivity.this, "网络请求失败: " + t.getMessage(), Toast.LENGTH_SHORT).show();
                Log.e("MyBuy", "Request Failed: " + t.getMessage());
            }
        });
    }
            ...
            public void onResponse(Call<GetProfileResponse> call, Response<GetProfileResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if ("success".equals(profileResponse.getStatus())) {
                        ...
                        Toast.makeText(BuyActivity.this, "获取用户信息失败: " + profileResponse.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                } else {
                    try {
                        // 从 errorBody 获取错误信息
            ...
            public void onResponse(Call<GetAttractivenessResponse> call, Response<GetAttractivenessResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if ("success".equals(getAttractivenessResponse.getStatus())) {
                        ...
                        Toast.makeText(BuyActivity.this, "获取用户信息失败: " + getAttractivenessResponse.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                } else {
                    try {
                        // 从 errorBody 获取错误信息
    ...
    private void generateLayout(Context context, List<GetMyBuyResponse.Trade> items) {
        if(items == null){
            return;
        }
        for (GetMyBuyResponse.Trade item : items) {
